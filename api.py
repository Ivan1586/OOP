...
class BirthDayField(DateField):
    def validate(self, value):
        ...
        if datetime.datetime.now() - birthday > datetime.timedelta(days=BIRTHDAY_LIMIT * 365):
            raise ValueError("The birthday should be no later than " f"{BIRTHDAY_LIMIT} years ago")
        return value
...
class Request(metaclass=MetaRequest):
    ...
    def __repr__(self):
        attributes = {name: getattr(self, name) for name in self.__dict__ if name[0:2] != "__"}
...
class MainHTTPHandler(BaseHTTPRequestHandler):
    ...
    def do_POST(self):
        ...
        if request:
            ...
            logging.info(
                "%s: %s %s" % (self.path, data_string.decode("utf8"), context["request_id"])
            )
